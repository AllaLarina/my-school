...
def view_classes(request):
    classes = Schoolkid.objects.all().values_list(
        'year_of_study', 'group_letter', 'entry_year').distinct()
    ...
    unique_class_letters = sorted(set([letter for year, letter, entry_year in classes]))
    context = {
...
def view_class_info(request, year, letter):
    ...
    context = {
        'year': int(year),
        'letter': letter,
        'entry_year': int(schoolkids[0].entry_year) if schoolkids else '',
